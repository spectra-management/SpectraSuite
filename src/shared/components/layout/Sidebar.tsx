...
import type { PermAction } from '@/shared/types/supabase'
...
const navItems: { key: string; to: string; icon: typeof LayoutDashboard; end: boolean; requires?: PermAction }[] = [
  ...
  { key: 'reports', to: '/nomina/reports', icon: FileBarChart2, end: false },
  { key: 'settings', to: '/nomina/settings', icon: Settings, end: false },
]

/**
 * Module sidebar. Desktop (md+): static, collapsible to an icon rail.
 * Mobile (<md): a 280px drawer that slides in over the content (controlled by
 * `mobileOpen`); the desktop collapse state is ignored there (labels always show).
 */
