...
import { Kanban, ArrowLeft, X } from 'lucide-react'
import { cn } from '@/shared/lib/utils'
import { useSettingsStore } from '@/shared/store/settingsStore'
import { useSidebarCollapsed } from '@/shared/hooks/useSidebarCollapsed'
import { SidebarToggle } from '@/shared/components/layout/SidebarToggle'

const navItems: { key: string; to: string; icon: typeof Kanban }[] = [
  { key: 'boards', to: '/tablero/boards', icon: Kanban },
]

export function TableroSidebar({ mobileOpen = false, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    ...
    >
      <div className={cn('flex items-center justify-between gap-1 border-b border-border py-3', collapsed ? 'px-4 md:px-2' : 'px-4')}>
        <Link to="/suite" title={collapsed ? t('suite.back') : undefined}
        ...
        <span className="hidden md:flex"><SidebarToggle collapsed={collapsed} onToggle={toggle} /></span>
      </div>
      ...
      <nav className="flex-1 space-y-1 overflow-auto p-3">
        {navItems.map(({ key, to, icon: Icon }) => (
          <NavLink key={key} to={to} onClick={onClose} title={collapsed ? t(`tablero.nav.${key}`) : undefined}
            className={({ isActive }) => cn(
              'relative flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-150',
              collapsed && 'md:justify-center md:gap-0 md:px-0',
              isActive ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'text-muted-foreground hover:bg-secondary hover:text-foreground',
            )}>
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute left-0 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-emerald-600" />}
                <Icon className={cn('h-4 w-4 shrink-0', isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground')} />
                <span className={cn(collapsed && 'md:hidden')}>{t(`tablero.nav.${key}`)}</span>
              </>
            )}
          </NavLink>
        ))}
      ...
      <div className={cn('flex items-center border-t border-border', collapsed ? 'gap-2 p-4 md:justify-center md:p-3' : 'gap-2 p-4')}>
        {company.logoBase64 ? (
          <img src={company.logoBase64} alt="logo" className="h-7 w-7 shrink-0 rounded-md object-contain" />
        ) : (
