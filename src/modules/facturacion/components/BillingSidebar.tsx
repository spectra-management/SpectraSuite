...
import { LayoutDashboard, Building2, FileText, BarChart3, Receipt, ArrowLeft, X } from 'lucide-react'
import { cn } from '@/shared/lib/utils'
import { useSettingsStore } from '@/shared/store/settingsStore'
import { useSidebarCollapsed } from '@/shared/hooks/useSidebarCollapsed'
import { SidebarToggle } from '@/shared/components/layout/SidebarToggle'

const navItems: { key: string; to: string; icon: typeof LayoutDashboard }[] = [
  { key: 'dashboard', to: '/facturacion/dashboard', icon: LayoutDashboard },
  { key: 'clients', to: '/facturacion/clients', icon: Building2 },
  { key: 'invoices', to: '/facturacion/invoices', icon: FileText },
  { key: 'reports', to: '/facturacion/reports', icon: BarChart3 },
]

/**
 * Billing module sidebar. Identical behaviour to the Nómina/RRHH sidebars
 * (collapsible icon rail on desktop, slide-in drawer on mobile).
 */
export function BillingSidebar({ mobileOpen = false, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
      ...
      <nav className="flex-1 space-y-1 overflow-auto p-3">
        {navItems.map(({ key, to, icon: Icon }) => (
          <NavLink
            ...
            to={to}
            onClick={onClose}
            title={collapsed ? t(`facturacion.nav.${key}`) : undefined}
            className={({ isActive }) =>
              cn(
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={cn(collapsed && 'md:hidden')}>{t(`facturacion.nav.${key}`)}</span>
              </>
            )}
          </NavLink>
        ))}
