...
import { FilePlus2, LayoutTemplate, History, FileText, ArrowLeft, X } from 'lucide-react'
import { cn } from '@/shared/lib/utils'
import { useSettingsStore } from '@/shared/store/settingsStore'
import { useSidebarCollapsed } from '@/shared/hooks/useSidebarCollapsed'
import { SidebarToggle } from '@/shared/components/layout/SidebarToggle'

const navItems: { key: string; to: string; icon: typeof FilePlus2 }[] = [
  { key: 'generate', to: '/documentos/generate', icon: FilePlus2 },
  { key: 'templates', to: '/documentos/templates', icon: LayoutTemplate },
  { key: 'history', to: '/documentos/history', icon: History },
]

/**
 * Documentos module sidebar. Identical behaviour to the Nómina/RRHH/Billing sidebars
 * (collapsible icon rail on desktop, slide-in drawer on mobile).
 */
export function DocumentosSidebar({ mobileOpen = false, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
      ...
      <nav className="flex-1 space-y-1 overflow-auto p-3">
        {navItems.map(({ key, to, icon: Icon }) => (
          <NavLink
            ...
            to={to}
            onClick={onClose}
            title={collapsed ? t(`documentos.nav.${key}`) : undefined}
            className={({ isActive }) =>
              cn(
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={cn(collapsed && 'md:hidden')}>{t(`documentos.nav.${key}`)}</span>
              </>
            )}
          </NavLink>
        ))}
