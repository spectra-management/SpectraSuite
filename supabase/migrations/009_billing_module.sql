...
--
-- Billing (facturación) module schema — FORWARD-LOOKING / MANUAL RUN ONLY.
--
...
--
-- RBAC: SELECT requires facturación 'view'; writes require 'edit'. The percentage
...
-- ── 1. Permission helper (aggregates legacy + role perms for facturación) ──────
-- Mirrors the app's hasModuleAccess('facturacion', action). super_admin / legacy
-- module_admin bypass. action ∈ ('view','edit','admin').
...
CREATE TABLE IF NOT EXISTS public.billing_clients (
  ...
  default_method TEXT NOT NULL DEFAULT 'hour' CHECK (default_method IN ('hour','fixed','percentage')),
...
CREATE TABLE IF NOT EXISTS public.billing_client_employees (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  client_id UUID NOT NULL REFERENCES public.billing_clients(id) ON DELETE CASCADE,
  employee_id TEXT NOT NULL,                 -- BambooHR/Spectra roster id
  method TEXT CHECK (method IN ('hour','fixed','percentage')),  -- NULL = inherit client default
...
CREATE TABLE IF NOT EXISTS public.billing_invoices (
  ...
  number TEXT NOT NULL DEFAULT '',            -- assigned on finalize
